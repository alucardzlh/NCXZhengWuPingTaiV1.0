...
public abstract class LeftMenuManagerService {
	 ...
	 */
	public void getLeftMenu(final Context paramActivity, final int paramInt,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("getLeftMenu", url, list);
					resultGroup = XmlUtil.LeftMenuXmlParse(result);
			...
			protected void onPostExecute(Integer paramInteger) {
				...
				if (paramInteger == 1) {// 获取到返回的信息
					...
					Toast.makeText(paramActivity, "获取导航栏失败！", Toast.LENGTH_LONG)
							.show();
					return;
				}
				LeftMenuManagerService.this.handlerLoginInfo(paramActivity,
