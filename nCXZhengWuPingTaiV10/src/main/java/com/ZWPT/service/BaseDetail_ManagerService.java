...
public abstract class BaseDetail_ManagerService {
	...
	public void getShowDetails(final Context parmasActivity,
		...
		final String url = parmasActivity.getResources().getString(
				com.ZWPT.activity.R.string.endpoint);// 调用的接口地址
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				try {
					if (Constants.title.equals(parmasActivity.getResources()
						...
						result_jbxx_ysl = downloadDB(
								"getYslReceiptByBizArchivesId", url, oList); // 基本信息
						result_wsbd = downloadDB("getBizArchivesOnlineForm",
								url, oList);
						result_fjck = downloadDB(
								"getBizAttachmentByBizArchivesId", url, oList);
						down_jbxx_ysl = true;
					} else {
						result_tab = downloadDB("getShowTabs", url, oList);// 第一个参数接口的方法名，第二个参数接口地址，第三个参数入参信息
					...
					if (down_jbxx) {
						result_jbxx = downloadDB("getBasicInfo", url, oList); // 基本信息
						Log.e("downloadDB", "result_jbxx");
					}
					if (down_ywbd) {
						result_ywbd = downloadDB("getBizArchivesForm", url,
								oList); // 业务表单
						Log.e("downloadDB", "result_ywbd");
					}
					if (down_sjdj_dzcl) {
						result_sjdj_dzcl = downloadDB("getBizReceiptInfo", url,
								oList);// 收件登记和电子材料
						Log.e("downloadDB", "result_sjdj_dzcl");
					}
					if (down_bzxx) {
						result_bzxx = downloadDB("getCorrectInfo", url, oList);
						Log.e("downloadDB", "result_bzxx");
					}
					if (down_ztxx) {
						result_ztxx = downloadDB("getPauseReasonInfo", url,
								oList);
						Log.e("downloadDB", "result_ztxx");
					}
					if (down_ffjl) {
						result_ffjl = downloadDB("getDeliveryRecordInfo", url,
								oList);// 发放记录
						Log.e("downloadDB", "result_ffjl");
					}
					if (down_wxts) {
						result_wxts = downloadDB("getTipInfo", url, oList); // 温馨提示
	...
	public void doPauseBizArchives(final Context paramActivity,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doPauseBizArchives", url, list);
					// resultGroup =
	...
	public void doStartBizArchives(final Context paramActivity,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doStartBizArchives", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doCorrectSendNote(final Context paramActivity,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doCorrectSendNote", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doCorrectAccept(final Context paramActivity,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doCorrectAccept", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doBizArchivesWithDrawal(final Context paramActivity,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doBizArchivesWithDrawal", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doBizArchives(final Context paramActivity, final int paramInt,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doBizArchives", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doSendReceipt(final Context paramActivity, final int paramInt,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doSendReceipt", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void doSendSM(final Context paramActivity, final int paramInt,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("doSendSM", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
	...
	public void noAccept(final Context paramActivity, final int paramInt,
		...
		BaseAsyncTask loacl = new BaseAsyncTask(paramActivity, true) {
			...
			protected Integer doInBackground(Integer[] paramArrayOfInteger) {
				...
				// 第二个参数是要传的参数，获取返回值
				try {
					result = downloadDB("noAccept", url, list);
					// resultGroup =
					// XmlUtil.getServiceSubjecrFormXmlParse(result);
				} catch (XmlPullParserException e) {
					// TODO 自动生成的 catch 块
